#!/usr/bin/env python3
"""
Train on combined CHEN11 + HOLO4K dataset.
Uses existing CHEN11 train/val splits and adds HOLO4K with 80/20 split.
"""
...
class ProteinDataset(Dataset):
    def __init__(self, samples):
        # samples: list of (protein_id, data_dir) tuples
    ...
    def __getitem__(self, idx):
        protein_id, data_dir = self.samples[idx]
        data = np.load(os.path.join(data_dir, f"{protein_id}.npz"))
        return {
...
# CHEN11 splits (existing)
with open('../data/splits/train.txt') as f:
    chen11_train = [(pid, CHEN11_DIR) for pid in f.read().splitlines()]
with open('../data/splits/val.txt') as f:
    chen11_val = [(pid, CHEN11_DIR) for pid in f.read().splitlines()]
...
split_idx    = int(len(holo4k_ids) * 0.8)
holo4k_train = [(pid, HOLO4K_DIR) for pid in holo4k_ids[:split_idx]]
holo4k_val   = [(pid, HOLO4K_DIR) for pid in holo4k_ids[split_idx:]]

train_samples = chen11_train + holo4k_train
val_samples   = chen11_val   + holo4k_val

print(f"Train: {len(chen11_train)} CHEN11 + {len(holo4k_train)} HOLO4K = {len(train_samples)} total")
print(f"Val:   {len(chen11_val)} CHEN11 + {len(holo4k_val)} HOLO4K = {len(val_samples)} total")

train_dataset = ProteinDataset(train_samples)
val_dataset   = ProteinDataset(val_samples)
...
n_pos, n_neg = 0, 0
for pid, data_dir in train_samples:
    path = os.path.join(data_dir, f"{pid}.npz")
